...
public class RequestProcessor {
	...
	public void WriteOff2200 (V5427HC1 currentClaim){
		...
		try {
			currentClaim.setHC1_COB_PRV_WRT_OFF(zero);
					...
					(currentClaim.getHC1_COB_INST_OR_PROF().equals("I") && currentClaim.getHC1_COB_COB_835_PROC_IND().equals("M")))
			{
				...
				/*
				 * Zeroing out medicare paid on claim leve  for institutional and professional medicare claims  
				 */
				...
				{
					currentClaim.setHC1_COB_MEDC_PAID_AMT(zero);
					System.out.println("currentClaim.getHC1_COB_MEDC_PAID_AMT() "+currentClaim.getHC1_COB_MEDC_PAID_AMT());
				}
				int cobDxCnt=1;
				do {
					/*
					 * For inst. and prof medicare claims zeroes to medicare paid amount on all line levels   
					 */
					if((currentClaim.getHC1_COB_INST_OR_PROF().equals("I") || currentClaim.getHC1_COB_INST_OR_PROF().equals("P"))
							&&
							currentClaim.getHC1_COB_COB_835_PROC_IND().equals("M"))
					{
						...
						//anjali:review,check if zero assigned
						currentClaim.getHC1_COB_LNE_DATA_AREA().get(cobDxCnt).setHC1_COB_LN_EOB_MEDC_PAID_AMT(zero);
						System.out.println("currentClaim.getHC1_COB_MEDC_PAID_AMT() "+currentClaim.getHC1_COB_MEDC_PAID_AMT());
					}
					...
					/*
					 * Provider write-off changed to zero at line   level for inst. medicare estimated claims
					 */
					if(currentClaim.getHC1_COB_COB_835_PROC_IND().equals("M") && currentClaim.getHC1_COB_INST_OR_PROF().equals("I"))
					{

						currentClaim.getHC1_COB_LNE_DATA_AREA().get(cobDxCnt).setHC1_COB_LN_COB_PRV_WRT_OFF(zero);
					}
			...
			if(currentClaim.getHC1_COB_INST_OR_PROF().equals("I") && currentClaim.getHC1_COB_COB_835_PROC_IND().equals("Y"))
			{
				...
				for (int i=1;i<=7;i++)
					currentClaim.getHC1_COB_LNE_DATA_AREA().get(i).setHC1_COB_LN_COB_PRV_WRT_OFF(zero);
			}
		} catch (Exception e) {
